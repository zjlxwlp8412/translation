...
def create_vocabs(file_list, vocab_file, language):
    word_counts = {
        "<PAD>": 100000000000,
        "<UNK>": 10000000000,
        "<S>": 1000000000,
            ...
            for line in reader:
                ...
                if language == "zh":
                    if len(line) <= 1:
                        ...
                        tokens = [token for token in list(line) if token != " "]
                else:
...
def create_inputs(file, word2idx, max_len, language):
    data = []
    with tf.gfile.Open(file, mode="r") as reader:
        for line in reader:
            line = line.strip("\n")
            ...
            if language == "zh":
                if len(line) <= 1:
                    ...
                    tokens = [token for token in list(line) if token != " "][:max_len]
            else:
...
def load_data(mode):
    try:
        ...
        create_vocabs([params.train_source_file, params.test_source_file], params.source_vocab_file, "zh")
        create_vocabs([params.train_target_file, params.test_target_file], params.target_vocab_file, "en")
        source_word2idx, source_idx2word = load_vocabs(params.source_vocab_file, params.min_count)
        target_word2idx, target_idx2word = load_vocabs(params.target_vocab_file, params.min_count)
    if mode == "train":
        source_inputs = create_inputs(params.train_source_file, source_word2idx, params.max_len, "zh")
        target_inputs = create_inputs(params.train_target_file, target_word2idx, params.max_len, "en")
    else:
        source_inputs = create_inputs(params.test_source_file, source_word2idx, params.max_len, "zh")
        target_inputs = create_inputs(params.test_target_file, target_word2idx, params.max_len, "en")
    source_vocab_size = len(source_word2idx.keys())
    target_vocab_size = len(target_word2idx.keys())
    return source_inputs, target_inputs, source_vocab_size, target_vocab_size, target_word2idx, target_idx2word, source_idx2word
